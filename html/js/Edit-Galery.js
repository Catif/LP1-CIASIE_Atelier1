let modal, newImagesListEl, tabFile, buttonSubmit;

function init(){
    modal = document.querySelector('.modal')
    newImagesListEl = document.querySelector('.images-list .new-picture')
    tabFile = []
...
function addImages(tab){
    ...
    try {
        if (tabPictureUpload.length > 0 && tabPictureUpload.length <= 4){
            ...
            newImagesListEl.innerHTML = `<p>Aucune image pour le moment...</p>`;
        }
    } catch (e) {
        buttonSubmit.disabled = true;
        newImagesListEl.innerHTML = `<p style="color:#FF0000">${e.message}</p>`;
...
function addImage(image){
    ...
    newImagesListEl.appendChild(div)
}
...
function initModal(){
    modal.querySelector('.footer [dataModal="valider"]')
        .addEventListener('click', (e) => {
            ...
            let divImage = newImagesListEl.querySelector('#file-' + rang)
...
function openModal(rang){
    ...
    let titleImage = newImagesListEl.querySelector(`input[name="title-image-${rang}`).value
    let tagImage = newImagesListEl.querySelector(`input[name="tag-image-${rang}`).value
